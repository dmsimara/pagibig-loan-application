...
public class SpouseDaoImpl implements SpouseDao {
    ...
    private static final String INSERT_SQL = "INSERT INTO spouse (spousePagibigMid, spouseName, spouseCitizenship, spouseDOB, spouseTin, spouseOccupation, spouseBusinessPhone, employerId, spousePosition, spouseYearsEmployment, pagibigMid, applicationNo) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
    private static final String UPDATE_SQL = "UPDATE spouse SET spouseName = ?, spouseCitizenship = ?, spouseDOB = ?, spouseTin = ?, spouseOccupation = ?, spouseBusinessPhone = ?, employerId = ?, spousePosition = ?, spouseYearsEmployment = ?, pagibigMid = ?, applicationNo = ? WHERE spousePagibigMid = ?";
    ...
    public void saveSpouse(Spouse spouse, String memberPagibigMid) throws Exception {
        try (Connection conn = DBConnection.getConnection()) {
            ...
            try {
                saveSpouse(conn, spouse, memberPagibigMid);
                conn.commit();
    ...
    public void saveSpouse(Connection conn, Spouse spouse, String memberPagibigMid) throws Exception {
        ...
        try (PreparedStatement updateStmt = conn.prepareStatement(UPDATE_SQL)) {
            ...
            updateStmt.setInt(11, spouse.getIntApplicationNo());
            updateStmt.setString(12, spouse.getStrSpousePagibigMid());
            ...
            if (affectedRows == 0) {
                try (PreparedStatement insertStmt = conn.prepareStatement(INSERT_SQL)) {
                    ...
                    insertStmt.setInt(12, spouse.getIntApplicationNo());
    ...
    @Override
    public Spouse getSpouseByPagibigMid(String pagibigMid) throws Exception {
        Spouse spouse = null;
        String sql = "SELECT spousePagibigMid, spouseName, spouseCitizenship, spouseDOB, spouseTin, spouseOccupation, spouseBusinessPhone, employerId, spousePosition, spouseYearsEmployment, pagibigMid FROM spouse WHERE pagibigMid = ?";
        try (Connection conn = DBConnection.getConnection();
             PreparedStatement stmt = conn.prepareStatement(sql)) {
            ...
            e.printStackTrace();
            throw new Exception("Error retrieving Spouse by pagibigMid: " + e.getMessage(), e);
        }
        return spouse;
    }
}
