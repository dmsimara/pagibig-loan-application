...
public class LoanApplicationDaoImpl implements LoanApplicationDao {
    ...
    private static final String INSERT_SQL = "INSERT INTO loan_application (housingAccountNo, purposeOfLoan, hasExistingApplication, housingApplicationNo, loanAmount, loanTerm, repricingPeriod, modeOfPayment, pagibigMid) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
    private static final String UPDATE_SQL = "UPDATE loan_application SET purposeOfLoan = ?, hasExistingApplication = ?, housingApplicationNo = ?, loanAmount = ?, loanTerm = ?, repricingPeriod = ?, modeOfPayment = ?, pagibigMid = ? WHERE housingAccountNo = ?";
    ...
    private void updateLoanApplication(Connection conn, LoanApplication loanApplication) throws SQLException {
        try (PreparedStatement stmt = conn.prepareStatement(UPDATE_SQL)) {
            ...
            stmt.setString(9, loanApplication.getHousingAccountNo());
            stmt.executeUpdate();
        }
    }
